...
from .ontology_dataloader import OntologyDataLoader


# Argument parser
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train a GNN model using ontology-based modeling with detection of important communities withing the ontology",
    )
    parser.add_argument(
        "--dataset", type=str, required=False, help="Path to the dataset directory",
    )
    parser.add_argument(
        "--n_communities", type=int, default=3, help="number of communities",
    )
    parser.add_argument(
        "--lambda_param",
        type=float,
        default=1.0,
        help="loss weighting between prediction and community metric (modularity)",
    )
    parser.add_argument(
        "--ontology_file", type=str, help="Path to the ontology file (ttl, rdf, owl)",
    )
    parser.add_argument(
        "--feature_class_map",
        type=str,
        default="mapping_titanic.json",
        help="Path feature-to-class map file (json",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save", type=bool, default=False, help="save the model checkpoint",
    )
...
def load_data(dataset_path, device, data_config):
    ...
    try:
        if "tcga" in dataset_path.lower():
            ...
            try:
                ...
                raise FileNotFoundError(
                    f"Error: The graph file '{graph_path}' was not found.",
                )
            # else:
            #     raise FileNotFoundError(f"Error: The graph file '{graph_path}' was not found.")

            try:
                matrix_connections = pd.read_csv(
                    os.path.join(dataset_path, "matrix_connection_truncated.csv"),
                )
                ...
                raise FileNotFoundError(
                    "Error: The feature->node connection matrix file was not found.",
                )

            def create_dataset(X, y, feature_map, edge_index, device=None):
                # Pre-transform the feature map (once, since it's constant)
                feature_map_t = torch.tensor(
                    feature_map, dtype=torch.float,
                )  # Transpose once

                X_tensor = torch.tensor(X, dtype=torch.float)  # Shape: [N, F]
                y_tensor = torch.tensor(
                    y, dtype=torch.float if len(y.shape) == 2 else torch.long,
                )
            ...
            train_dataset = create_dataset(
                X_train, y_train, feature_map, edge_index, device=device,
            )
            test_dataset = create_dataset(
                X_test, y_test, feature_map, edge_index, device=device,
            )
        ...
        if "titanic" in dataset_path.lower():
            ...
            try:
                ...
                class_mapping_path = os.path.join(
                    dataset_path, data_config["feature_class_map"],
                )
                ...
                all_classes = sorted(
                    set(cls for cls_list in mapping.values() for cls in cls_list).union(
                        relation_classes,
                    ),
                )
                ...
                for source, target, relation_type in relation_matrix:
                    if source in class_to_idx and target in class_to_idx:
                        class_relationships.append(
                            (class_to_idx[source], class_to_idx[target]),
                        )
                ...
                for i in range(len(df)):  # assuming `df` is your sample-wise data
                    ...
                    edge_index = (
                        torch.tensor(class_relationships, dtype=torch.long)
                        .t()
                        .contiguous()
                    )
                ...
                if n_samples:
                    ...
                    test_loader = DataLoader(
                        data_list[n_samples:], batch_size=16, shuffle=False,
                    )
                else:
                    train_split = 0.7
                    train_loader = DataLoader(
                        data_list[: int(train_split * len(data_list))],
                        batch_size=16,
                        shuffle=True,
                    )
                    test_loader = DataLoader(
                ...
                raise FileNotFoundError(
                    "Error: The file was not found or error reading it.",
                )
        ...
        raise FileNotFoundError(
            f"Error: The dataset file '{dataset_path}' was not found.",
        )
        ...
        raise Exception(f"An unexpected error occurred: {e!s}")
