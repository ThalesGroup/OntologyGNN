...
from .gnn_model import OntologyCommunityDetection
...
def train_model(
    ...
    for name, param in model.named_parameters():
        if name in learning_rates:
            ...
            all_params.append(
                {"params": [param], "lr": training_specs["lr"]},
    ...
    for epoch in range(epochs):
        ...
        for data in train_loader:
            ...
            modularity = torch.mean(
                compute_modularity_loss_fast(
                    communities, train_loader.dataset[0].edge_index.to(device), n_nodes,
                ),
            )
            loss = (
                criterion(output_batch.squeeze(1), data.y)
                + params["lambda"] * modularity
            )
